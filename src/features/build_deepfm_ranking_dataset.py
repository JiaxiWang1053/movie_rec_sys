...
def extract_year_from_title(title: str):
    """
    从标题中提取年份，例如 Toy Story (1995) -> 1995
    """
...
def build_user_genre_history(split_dict: dict, item_genre_ids_map: dict):
    """
    构造每个用户历史上的 genre 集合（使用 train history）
    """
    user_genre_history = {}
...
def compute_retrieval_scores_for_candidates(
    ...
    """
    用训练好的召回模型给候选集打分，作为排序特征 retrieval_score
    """
    model.eval()
...
def build_deepfm_ranking_dataset(
    data_path: str,
    stage: str = "valid",
    ...
    - sparse_feature_info: 稀疏特征词表大小
    - dense_feature_cols: 连续特征列
    - item_genre_ids_map: 每个 item 对应的 genre id 列表
    - num_genres: genre 总数
    """
