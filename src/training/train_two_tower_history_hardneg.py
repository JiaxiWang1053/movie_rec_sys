...
from src.data.build_retrieval_train_data_hardneg import build_retrieval_train_data_hardneg
...
def train_two_tower_history_hardneg(
    data_path: str,
    ...
    random_seed: int = 42,
    device: str = "cuda",
    save_path: str = "checkpoints/two_tower_history_hardneg.pth"
):
    _, users, movies = load_ml_1m(data_path)
    num_users = users["user_id"].max() + 1
    num_items = movies["movie_id"].max() + 1

    retrieval_train_df = build_retrieval_train_data_hardneg(
        data_path=data_path,
