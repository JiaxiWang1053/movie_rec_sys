...
class TwoTowerHistoryMLPRetrievalModel(nn.Module):
    """
    带用户历史 pooling + MLP 的双塔召回模型
    ...
    def encode_user(
        ...
        用户编码：
        1. user_id embedding
        2. history item embedding mean pooling
        3. 拼接后输入 MLP
        """
        ...
        user_input = torch.cat([user_id_emb, history_mean_emb], dim=1) # [B, 2D]
