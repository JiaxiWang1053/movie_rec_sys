...
from src.models.two_tower_history_mlp import TwoTowerHistoryMLPRetrievalModel
...
def main():
    ...
    model = TwoTowerHistoryMLPRetrievalModel(
        num_users=num_users,
        num_items=num_items,
        embedding_dim=64,
        hidden_dim=128
    ...
    model.load_state_dict(
        torch.load("checkpoints/two_tower_history_mlp_mixedneg.pth", map_location=device)
    )
    print("模型加载完成：two_tower_history_mlp_mixedneg.pth")
    ...
    print("\n双塔 + History + MLP + Mixed Negative 召回结果：")
